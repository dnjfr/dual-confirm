    ...
    try:
        ...
        if session.get('role') == 'advisor':
            ...
            if redis_get(redis_users_sessions, selection_key) != "selected":
                emit('update_passkeys_pairs', {"error": "Unselected client"})
                return
        ...
        # Retrieve passkeys and TTL for user and advisor
        ...
        emit('update_passkeys_pairs', {"error": str(e)})